...
from util.cluster_and_log_utils import split_cluster_acc_v2
...
def train(student, train_loader, test_loader, unlabelled_train_loader, args):
    ...
    # criterion_instance = InstanceLoss(args.batch_size, 0.5, 'cuda').to('cuda')
    # criterion_cluster = ClusterLoss(200, 1.0, 'cuda').to('cuda')
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                images_i, images_j = images.chunk(2)[0], images.chunk(2)[1]
                z_i, z_j, c_i, c_j = student(images_i, images_j)

                # loss_instance = criterion_instance(z_i, z_j)
                # loss_cluster = criterion_cluster(c_i, c_j)
                con_logits_instance, con_labels_instance = info_nce_logits(features=torch.cat((z_i,z_j),dim=0))
                loss_instance = torch.nn.CrossEntropyLoss()(con_logits_instance, con_labels_instance)

                con_logits_cluster, con_labels_cluster = info_nce_logits(features=torch.cat((c_i,c_j),dim=0))
                loss_cluster = torch.nn.CrossEntropyLoss()(con_logits_cluster, con_labels_cluster)

                # avg_probs = torch.cat((c_i,c_j),dim=0).mean(dim=0)
                ...
                proj_instance = torch.cat((z_i[mask_lab].unsqueeze(1),z_j[mask_lab].unsqueeze(1)), dim=1)
                # student_proj = torch.nn.functional.normalize(student_proj, dim=-1)
                sup_con_labels = class_labels[mask_lab]
                loss_instance_sup = SupConLoss()(proj_instance, labels=sup_con_labels)

                proj_cluster = torch.cat((c_i[mask_lab].unsqueeze(1),c_j[mask_lab].unsqueeze(1)), dim=1)
                # student_proj = torch.nn.functional.normalize(student_proj, dim=-1)
                sup_con_labels = class_labels[mask_lab]
                loss_cluster_sup = SupConLoss()(proj_cluster, labels=sup_con_labels)

                # if torch.isnan(loss_instance) or torch.isinf(loss_instance):
                #     loss_instance = torch.tensor(0.0, device=loss_instance.device)
                # if torch.isnan(loss_cluster) or torch.isinf(loss_cluster):
                #     loss_cluster = torch.tensor(0.0, device=loss_cluster.device)
                # if torch.isnan(loss_instance_sup) or torch.isinf(loss_instance_sup):
                #     loss_instance_sup = torch.tensor(0.0, device=loss_instance_sup.device)
                # if torch.isnan(loss_cluster_sup) or torch.isinf(loss_cluster_sup):
                #     loss_cluster_sup = torch.tensor(0.0, device=loss_cluster_sup.device)
                ...
                pstr = ''
                pstr += f'loss_instance: {loss_instance.item():.4f} '
                pstr += f'loss_instance_sup: {loss_instance_sup.item():.4f} '
                pstr += f'loss_cluster: {loss_cluster.item():.4f} '
                pstr += f'loss_cluster_sup: {loss_cluster_sup.item():.4f} '
                pstr += f'ne_loss: {ne_loss.item():.4f} '

                loss = 0
                loss += (1 - args.sup_weight) * loss_instance + args.sup_weight * loss_instance_sup
                loss += (1 - args.sup_weight) * loss_cluster + 0.05 * loss_cluster_sup
                loss += ne_loss
        ...
        all_acc, old_acc, new_acc = test(unlabelled_train_loader, student)
...
def test(loader, model):
    ...
    labels_vector = []
    mask = np.array([])
    for batch_idx, (images, label, _) in enumerate(tqdm(loader)):
        images = images.cuda(non_blocking=True)
        with torch.no_grad():
            c = model.forward_cluster(images)
        c = c.detach()
        feature_vector.append(c.cpu().detach().numpy())
        labels_vector.append(label.cpu().numpy())
        mask = np.append(mask, np.array([True if x.item() in range(len(args.train_classes)) else False for x in label]))

    feature_vector = np.concatenate(feature_vector)
    labels_vector = np.concatenate(labels_vector)

    mask = mask.astype(bool)
    feature_vector = feature_vector.astype(int)
    labels_vector = labels_vector.astype(int)
    total_acc, old_acc, new_acc = split_cluster_acc_v2(labels_vector,feature_vector,mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', default='cc-1', type=str)
    ...
    model = Network(backbone, 128, args.mlp_out_dim).to(device)
    # ----------------------
    # TRAIN
    # ----------------------
