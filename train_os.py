    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                ##os negative
                # # 定义一个大的负值
                # negative_large_value = -torch.exp(torch.tensor(9.0)).to(teacher_out.device)
                # class_labels_two = torch.cat([class_labels for _ in range(2)], dim=0)

                # # 将类别列表转换为张量
                # unlabeled_classes_tensor = torch.tensor(args.unlabeled_classes).to(teacher_out.device)
                # train_classes_tensor = torch.tensor(args.train_classes).to(teacher_out.device)

                # # 对每一个batch进行操作
                # for i in range(teacher_out.size(0)):
                #     # 获取top50值和它们的索引
                #     values, indices = torch.topk(teacher_out[i], 10)

                #     # 根据class_labels的值进行判断和替换
                #     if class_labels_two[i] in unlabeled_classes_tensor:
                #         # 找到索引在train_classes的位置
                #         mask = torch.isin(indices, train_classes_tensor)
                #         teacher_out[i][indices[mask]] = negative_large_value
                #     else:
                #         # 找到索引在unlabeled_classes的位置
                #         mask = torch.isin(indices, unlabeled_classes_tensor)
                #         teacher_out[i][indices[mask]] = negative_large_value
                ##os negative
                ######
                ...
                pstr += f'loss_os: {contrastive_loss.item():.4f} '
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', default='simgcd-scars-osloss-true', type=str)
    ...
    init_experiment(args, runner_name=['os'])
